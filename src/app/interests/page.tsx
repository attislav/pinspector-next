    ...
    try {
      for (let i = 0; i < selectedIdeas.length; i++) {
        ...
        try {
          ...
          console.error(`Error rescraping ${idea.name}:`, err);
        }
      ...
      // Refresh data after rescraping
      await fetchIdeas();
      setSelectedIds(new Set());
    } finally {
      setRescraping(false);
      setRescrapeProgress({ current: 0, total: 0, currentName: '' });
    }
  };
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `pinspector-export-${new Date().toISOString().split('T')[0]}.csv`;
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {formatNumber(total)} Interessen in der Datenbank
          </p>
        </div>
        <div className="flex gap-2">
          ...
          >
            ...
            {selectedIds.size > 0 ? `${selectedIds.size} exportieren` : 'Alle exportieren'}
          </button>
        </div>
      </div>
        ...
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-blue-600 text-sm">
              {rescrapeProgress.current} / {rescrapeProgress.total}
            </span>
          </div>
          <div className="w-full bg-blue-200 rounded-full h-2 mb-2">
            <div
              className="bg-blue-600 h-2 rounded-full transition-all duration-300"
              style={{ width: `${(rescrapeProgress.current / rescrapeProgress.total) * 100}%` }}
            />
          </div>
          {rescrapeProgress.currentName && (
            <p className="text-blue-600 text-sm truncate">
              Aktuell: {rescrapeProgress.currentName}
            </p>
          )}
        </div>
      )}
      ...
      <div className="mb-6 space-y-4">
        ...
        {showFilters && (
          <div className="p-4 bg-gray-50 rounded-lg space-y-4">
            {/* Category Filters - prominently displayed */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    const newMain = e.target.value || undefined;
                    setFilters({ ...filters, mainCategory: newMain, subCategory: undefined });
                    setPage(1);
                  }}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    setFilters({ ...filters, subCategory: e.target.value || undefined });
                    setPage(1);
                  }}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-2 border-t border-gray-200">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFilters({ ...filters, minWords: e.target.value ? parseInt(e.target.value) : undefined })}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFilters({ ...filters, maxWords: e.target.value ? parseInt(e.target.value) : undefined })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg"
                  placeholder="10"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFilters({ ...filters, minSearches: e.target.value ? parseInt(e.target.value) : undefined })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg"
                  placeholder="0"
                />
              </div>
              <div>
                ...
                <input
                  type="number"
                  value={filters.maxSearches || ''}
                  onChange={(e) => setFilters({ ...filters, maxSearches: e.target.value ? parseInt(e.target.value) : undefined })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg"
                  placeholder="1000000"
                />
              </div>
              <div>
                ...
                <select
                  value={filters.sortBy || 'last_scrape'}
                  onChange={(e) => setFilters({ ...filters, sortBy: e.target.value as InterestFilters['sortBy'] })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg"
                >
              ...
              <div className="col-span-2 flex items-end">
                <button
                  onClick={() => {
                    setFilters({ search: '', sortBy: 'searches', sortOrder: 'desc' });
                    setPage(1);
                  }}
              ...
              <tr>
                ...
                <th className="px-4 py-3 text-left">
                  <button
                    ...
                    className="flex items-center gap-1 text-sm font-semibold text-gray-700 hover:text-red-700 transition-colors"
                  >
                    Name
                ...
                <th className="px-4 py-3 text-right">
                  <button
                    ...
                    className="flex items-center gap-1 text-sm font-semibold text-gray-700 hover:text-red-700 transition-colors ml-auto"
                  >
                    Suchanfragen
                  ...
                  <tr key={idea.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="text-sm text-gray-600 truncate max-w-xs">
                        {idea.seo_breadcrumbs?.map((crumb: string | { name: string }) =>
                          typeof crumb === 'string' ? crumb : crumb.name
                        ).join(' > ') || '-'}
                      </div>
                    </td>
