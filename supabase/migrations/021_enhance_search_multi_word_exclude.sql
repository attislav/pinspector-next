-- Enhance search: multi-word AND search + exclude keywords
-- Drop existing function (signature changes - adding p_exclude_keywords)
...
DECLARE
  ...
  exclude_words TEXT[];
BEGIN
  -- Split search into individual words (space-separated, AND logic)
  ...
  -- Split exclude keywords (comma-separated)
  IF p_exclude_keywords IS NOT NULL AND p_exclude_keywords <> '' THEN
    exclude_words := string_to_array(p_exclude_keywords, ',');
    -- Trim whitespace from each keyword
    FOR i IN 1..array_length(exclude_words, 1) LOOP
      exclude_words[i] := trim(exclude_words[i]);
    END LOOP;
    -- Remove empty strings
    exclude_words := array_remove(exclude_words, '');
  END IF;
  ...
  FROM public.ideas i
  WHERE
    -- Multi-word search: ALL words must appear in name (AND logic)
    (search_words IS NULL OR NOT EXISTS (
      SELECT 1 FROM unnest(search_words) sw
      WHERE i.name NOT ILIKE '%' || sw || '%'
    ))
    -- Exclude keywords: NONE of the exclude words may appear in name
    AND (exclude_words IS NULL OR NOT EXISTS (
      SELECT 1 FROM unnest(exclude_words) ew
      WHERE i.name ILIKE '%' || ew || '%'
    ))
