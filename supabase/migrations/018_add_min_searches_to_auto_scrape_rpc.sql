-- Drop old function (signature changes - adding p_min_searches)
DROP FUNCTION IF EXISTS get_auto_scrape_candidate(text, integer, text);

CREATE OR REPLACE FUNCTION get_auto_scrape_candidate(
  p_language TEXT,
  p_min_age_days INTEGER DEFAULT 30,
  p_name_contains TEXT DEFAULT NULL,
  p_min_searches INTEGER DEFAULT 1
)
RETURNS TABLE(
  id TEXT,
  name TEXT,
...
BEGIN
  ...
  WITH candidates AS (
    ...
    WHERE i.language = p_language
      ...
      AND i.searches >= p_min_searches
      AND i.name !~ '\b20(1[0-9]|2[0-4])\b'
      AND (p_name_contains IS NULL OR i.name ILIKE '%' || p_name_contains || '%')
      -- Exclude ideas currently being scraped (running sync_log entries)
      AND i.id NOT IN (
        SELECT sl.idea_id FROM public.sync_log sl
        WHERE sl.status = 'running'
          AND sl.started_at > NOW() - INTERVAL '10 minutes'
      )
  ),
  stats AS (
    SELECT
